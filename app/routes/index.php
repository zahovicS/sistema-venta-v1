...
app()->registerMiddleware('web-auth', function () {
    auth()->guard('auth');
    (new \App\Middleware\UserHasStoreMiddleware())->call();
    (new \App\Middleware\UserHasRolesMiddleware())->call();
});

app()->registerMiddleware('permissionMiddleware', function () {
    (new \App\Middleware\UserHasPermissionMiddleware())->call();
});
